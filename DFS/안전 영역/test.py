...
for i in range(n):
    AreaList.append(list(map(int,sys.stdin.readline().split())))
    if max(AreaList[i]) > maxVal:
        maxVal = max(AreaList[i])
...
def dfs(x, y, h):
    ...
    if AreaList[y][x] > h and visited[y][x] == 0:
        visited[y][x] = 1       
        dfs(x + 1, y, h)
        dfs(x, y + 1, h)
        dfs(x - 1, y, h)
