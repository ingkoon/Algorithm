    ...
    for i in range(y, y + n):
        for j in range(x, x + n):
            if board[y][x] != board[i][j]:
                ...
                dfs(x, y + nextN, nextN)
                dfs(x, y + (nextN * 2),nextN)

                dfs(x + nextN, y, nextN)
                dfs(x + (nextN * 2) , y, nextN)

                dfs(x + nextN, y + nextN, nextN)
                dfs(x + (nextN * 2), y + nextN, nextN)                                                
                dfs(x + nextN, y + (nextN * 2), nextN)
                dfs(x + (nextN * 2), y + (nextN * 2),nextN)
                return
